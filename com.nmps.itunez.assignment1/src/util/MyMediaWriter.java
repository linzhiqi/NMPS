...
class MyMediaWriter extends AMediaCoderMixin implements IMediaWriter {
	...
	static {
		com.xuggle.ferry.JNIMemoryManager
				.setMemoryModel(com.xuggle.ferry.JNIMemoryManager.MemoryModel.NATIVE_BUFFERS);
	}
	...
	MyMediaWriter(String url, IMediaReader reader) {
	...
	MyMediaWriter(String url, IContainer inputContainer) {
	...
	public int addAudioStream(int inputIndex, int streamId, int channelCount,
			int sampleRate) {
		...
		if (format == null && (url == null || url.length() < 0))
			throw new IllegalArgumentException(
					"Cannot guess codec without container or url");
		ICodec codec = ICodec.guessEncodingCodec(format, null, url, null,
				ICodec.Type.CODEC_TYPE_AUDIO);
		if (codec == null)
			throw new UnsupportedOperationException(
					"could not guess audio codec");
	...
	public int addVideoStream(int inputIndex, int streamId, int width,
			int height) {
		return addVideoStream(inputIndex, streamId, (IRational) null, width,
				height);
	}
	...
	public int addVideoStream(int inputIndex, int streamId, ICodec.ID codecId,
		...
		return addVideoStream(inputIndex, streamId, codecId, null, width,
				height);
	}
	...
	public void encodeVideo(int streamIndex, BufferedImage image,
	...
	public void encodeAudio(int streamIndex, short[] samples, long timeStamp,
			TimeUnit timeUnit) {
		// verify parameters
		if (null == samples)
			throw new IllegalArgumentException("NULL input samples");
	...
	private boolean addStreamFromContainer(int inputStreamIndex) {
		...
		case CODEC_TYPE_AUDIO:
			addAudioStream(inputStream.getIndex(), inputStream.getId(),
					...
					ICodec.ID.CODEC_ID_PCM_MULAW,
					inputCoder.getChannels(), inputCoder.getSampleRate());
			System.out.println("using pcm u");
			break;
